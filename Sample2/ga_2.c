...
/* gtype (遺伝子コード) の実数 GA のための実装 */
/* gtype (int 配列) を作り最初のアドレスを返す */
...
gtype_t mk_random_gtype(int code_length, int code_max)
{
    ...
    int i;
    for(i=0;i<code_length;i++) ptr[i] = rand()%(code_max+1);
    return ptr;
}
 ...
 * 4、7 節で説明したので省略
 */
void encode_gtype(double value, gtype_t gtype,
    int code_length, double min, double max)
{
    double gap = max -min;
    ...
    while ( i < code_length ){
        ...
        /* グレイ表現への変換 */
        /* バイナリ表現と、元のバイナリを右に1シフトしたもののXORをとる */
        if (GRAY == 1){
            tmp = gtype[i];
            gtype[i] = (pre_code) ^ (gtype[i]);
            pre_code = tmp; /* 変換前の値を、次の桁の変換に用いる */
        }
        position++;
        i++;
    }
    ...
    double min, double max)
{
    ...
    int position = 1;
    int pre_code = 0;
    /* 1つ上位の桁の表現(バイナリ), バイナリとグレイの変換に必要 */

    int i=0;
    /* グレイ表現の解釈 */
    /* 変換されたバイナリの1つ上位の桁の表現との排他的論理和をとる */
    if (GRAY==1){
        while (i<code_length){
...
/* 線形リスト用の隣接した要素の入れ替え。引数は先頭の individual_t のアドレス */
...
int mk_children_genes(individual_t child1, individual_t child2,
                      ...
                      int code_length,int code_max,double pm)
{
    ...
    mutateCount =
        mutate_gtype(child1->gtype,code_length,code_max,pm);
    mutateCount +=
        mutate_gtype(child2->gtype,code_length,code_max,pm);
    return mutateCount;
}

/* GA 集団の作成、初期化を行う */
ga_population_t mk_init_ga_population(int population_size,
                                      int code_length, int code_max)
{
    ga_population_t population =
        my_malloc(sizeof(struct ga_population));
    population->pselect =
        (double*)my_malloc(sizeof(double)*population_size);
...
/* 指定した文字 ch を指定した長さ length だけ繰り返す関数 */
...
/* 集団を表示する。 */
/* 左から、世代数、親のインデックス、交叉点, gtype, ptype, fitness を表示する。 */
/* また、最後に突然変異の回数を表示する。 */
void print_population(ga_population_t population)
{
    ...
    while(member != NULL){
        printf("%3d (%3d,%3d) %3d  ", i,
               member->parent1, member->parent2, member->cross_point);
...
/* 適合度を出力
 * 最大, 平均, 最小, population->max_fitness, population->avg_fitness,
 * CSV 形式にする
 */
void print_fitness(ga_population_t population)
{
    printf("%f, %f, %f, %f, ",
           ...
           population->min_fitness, population->genes->ptype);
...
void calc_fitness(ga_population_t population,
...
individual_t select_parent_roulette(ga_population_t population)
{
    ...
    individual_t parent = population->genes;
...
/* 親個体の選択、param.h の SELECTION_METHOD によって
 * ルーレット選択かトーナメント選択を行う
 */
...
/* 新しい世代の生成
 * new_population のメモリ領域はすでに確保してあるとする
 * 必ずソート済みの population を渡すこと
 */
...
main()
{
    ...
    int i;
    if(PRINT_FITNESS==1)
        printf("#generation,max fitness, avg fitness, min fitness, best individual_ptype, best individual_gtype(n)");
    ...
    for(i=0;i <= GENERATION;i++){
        ...
        // 現在の世代 parent_group から次世代 child_group を作る
        generate_population(child_group,parent_group,GAP,
                            ELITE_RATE, MUTATE_PROB);
        // 世代を入れ替える
        parent_group = child_group; // (typo: proup -> group に修正)
  }
  return 0;
}
